...
- **시간 감각**: 장꾸가 현재 시각과 "지난 대화로부터 얼마나 지났는지"를 알고 있어서,
  몇 시간 만에 말을 걸면 "흥, 이제야 온 거야?" 하고 티를 내고 늦은 밤엔 걱정도 해줍니다
- **화면 분석**: "내 화면 봐줘", "지금 나 뭐 하고 있게?" 하면 화면을 캡처해
  Gemini 비전으로 분석하고 캡처 시각과 함께 알려줍니다
  (요청할 때만 캡처하며, 분석을 위해 화면 이미지가 Gemini API로 전송됩니다)
- **제한적인 컴퓨터 제어** (화이트리스트, 임의 명령 실행 없음):
  - 앱 열기(메모장·계산기·그림판·탐색기·설정 등), 웹사이트 열기, 웹 검색
  - 볼륨 조절, 음소거, 미디어 재생/일시정지/곡 넘기기
  - 현재 시각·배터리·CPU·메모리 알려주기, 스크린샷 저장, 화면 분석
...
| 탭 | 항목 |
| --- | --- |
| 🐾 일반 | 장꾸 크기, 걷는 속도, 활동성(게으름↔활발), 맨 위 고정, Windows 시작 시 자동 실행 |
| ✨ 확장 기능 | 아래 9가지 확장 기능을 **개별 on/off** (기본값 전부 켜짐) |
| 🤖 AI | API 키, 모델 선택, **연결 테스트**(장꾸가 한마디 해줌), 대화 기록 지우기 |
| ℹ️ 정보 | 버전, API 키 발급 페이지 열기, 종료 |

**✨ 확장 기능 탭의 토글** — 기본 기능(펫 상주·자율 행동·채팅·컴퓨터 제어)은 항상
동작하고, 그 위에 얹힌 확장 기능은 취향대로 끌 수 있습니다:

| 토글 | 끄면 |
| --- | --- |
| 🗨️ 혼잣말 | 한가할 때 혼잣말 말풍선 안 띄움 |
| 🔋 배터리 경고 | 배터리 15% 경고 안 함 |
| 🕐 시간대 인사 | 점심/퇴근/심야 인사 안 함 |
| 💾 대화 기억 | 대화를 파일에 저장하지 않음 (실행 중에만 기억) |
| ⏰ 시간 감각 | 장꾸가 시각/경과 시간을 모름 |
| 👁️ 화면 분석 | "내 화면 봐줘" 거절 (화면 캡처 안 함) |
| 💬 말풍선 답변 | 채팅창 닫으면 답변을 말풍선으로 전달 안 함 |
| 🪂 낙하 애니메이션 | 드래그 후 즉시 작업표시줄로 복귀 |
| ↔️ 방향 유지 | 멈추면 포즈 원본 방향으로 표시 |
